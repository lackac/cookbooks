...
Server and Client is tested on Ubuntu 9.10.
...
Server using server_passenger:

* nginx::server (lackac/cookbooks)
* passenger::nginx (lackac/cookbooks)
...
On servers, this specifies the URLs the server expects, plus it is used in the server_ssl_req as the canonical name (CN) and in server_proxy and server_passenger for the vhost name.
...
Used by the server_proxy and server_passenger recipes, this attribute can be used to set up a self-signed SSL certificate automatically using openssl. Fields:
...
=== With Passenger (chef::server_passenger):

If you would like to use Passenger for the server, use the chef::server_passenger recipe.

When using this recipe, clients should have the url_type attribute set to "https".

You will need to edit the server_ssl_request attribute so the certificate is generated correctly.

The recipe itself will set up two nginx virtual hosts, one for the server and one for OpenID:
